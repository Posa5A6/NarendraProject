...
function header(){
  return `<header class="site-header"><div class="container nav"><a class="brand-wrap" href="index.html" aria-label="Hotel Rajshri home"><img class="brand-logo" src="${SITE.logo}" alt="Hotel Rajshri · Pure Veg A/C Restaurant"></a><button class="menu-toggle" type="button" aria-label="Open navigation" aria-expanded="false" aria-controls="site-nav">☰</button><nav class="nav-links" id="site-nav"><a href="index.html">Home</a><a href="menu.html">Menu</a><a class="nav-train train-order-trigger" href="#train-order-dialog" title="Available only for trains stopping at Adoni. Online ordering is coming soon.">Train delivery <span aria-hidden="true">&#9432;</span></a><a href="about.html">About</a><a href="#site-footer">Contact</a><a class="nav-order" href="order.html">Order food</a></nav></div></header>`;
}
function railIcon(){return `<svg viewBox="0 0 48 48" fill="none" aria-hidden="true"><path d="M15 7h18a4 4 0 0 1 4 4v19a9 9 0 0 1-9 9H20a9 9 0 0 1-9-9V11a4 4 0 0 1 4-4Z" stroke="currentColor" stroke-width="1.7"/><path d="M17 15h14v10H17zM16 29h.01M32 29h.01M17 39l-4 5M31 39l4 5M9 44h30" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>`}
function trainOrderDialog(){return `<dialog class="train-order-dialog" id="train-order-dialog" aria-labelledby="train-order-title"><button class="train-order-close" type="button" aria-label="Close train delivery details">×</button><div class="train-order-icon">${railIcon()}</div><p class="lux-kicker dark">Rail journey service</p><h2 id="train-order-title">Food for<br><em>your journey.</em></h2><p>Fresh pure vegetarian meals for trains stopping at Adoni. Online train ordering is being developed and will be available soon.</p><span class="train-caution" tabindex="0" data-tooltip="Train delivery will be available only for trains stopping at Adoni. Online ordering is coming soon.">Adoni station only <b aria-hidden="true">i</b></span><button class="lux-solid-button train-coming-soon" type="button" disabled>Order for your train <span>Coming soon</span></button></dialog>`}
function footer(){return `<footer><div class="container footer-grid"><div><img class="footer-logo" src="${SITE.logo}" alt="Hotel Rajshri"><p>${SITE.tagline}. Fresh vegetarian food for locals, families, and railway travellers.</p></div><div><strong>Explore</strong><div class="footer-links"><a href="index.html">Home</a><a href="about.html">Our story</a><a href="menu.html">Menus</a><a href="gallery.html">Gallery</a><a href="order.html">Order food</a></div></div><div class="footer-contact"><strong>Contact</strong><p>${SITE.address}</p><p><span>Open daily</span>8:00 AM – 10:30 PM</p><p><span>Owner</span>Mahendra Kumar J Sha</p><a class="footer-phone" href="tel:+919959461888" aria-label="Call Hotel Rajshri at 099594 61888"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6.6 3.4 9.5 6.3 7.7 8.8a16.1 16.1 0 0 0 7.5 7.5l2.5-1.8 2.9 2.9-1.8 3.2c-.4.7-1.2 1-1.9.8C9.9 19.4 4.6 14.1 3.6 7.1c-.1-.7.1-1.5.8-1.9l2.2-1.8Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"/></svg><span>099594 61888</span></a><a class="btn btn-primary" href="${SITE.maps}" target="_blank" rel="noopener">Get directions</a></div></div><div class="container footer-bottom"><span>© ${new Date().getFullYear()} Hotel Rajshri</span><span>Pure vegetarian dining in Adoni</span></div></footer>`}
function shell(){
  ...
  if(!$('#train-order-dialog'))document.body.insertAdjacentHTML('beforeend',trainOrderDialog());
  ...
  const sync=()=>{if(!headerEl)return;headerEl.classList.toggle('on-dark',!!dark&&scrollY<Math.max(420,(dark.offsetHeight||0)-100))}; sync(); addEventListener('scroll',sync,{passive:true});
  const trainDialog=$('#train-order-dialog');
  $$('.train-order-trigger').forEach(trigger=>trigger.addEventListener('click',event=>{event.preventDefault();trainDialog?.showModal()}));
  $('.train-order-close',trainDialog)?.addEventListener('click',()=>trainDialog.close());
  trainDialog?.addEventListener('click',event=>{if(event.target===trainDialog)trainDialog.close()});
}
